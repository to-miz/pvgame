...
struct MemberArrayView {
	...
	struct iterator {
		...
		{
			auto result = (T*)ptr;
			assert_alignment( result, alignof( T ) );
			return *result;
		}
		...
		bool operator!=( range_iterator other ) { return this->ptr != other.ptr; }
		bool operator==( range_iterator other ) { return this->ptr == other.ptr; }
	};
};

template<class U, class T > MemberArrayView< T > makeMemberArrayView_( Array< U > array, uint16 memberOffset )
{
	return MemberArrayView< T >{(char*)array.data(), memberOffset, sizeof( U ), array.size()};
}

#define makeMemberArrayView( array, member )                               \
	makeMemberArrayView_< typeof( typeof( array )::value_type::member ) >( \
	    array, offsetof( typeof( array )::value_type, typeof( array )::member ) )